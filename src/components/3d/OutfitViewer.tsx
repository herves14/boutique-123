'use client'
// src/components/3d/OutfitViewer.tsx
...
export default function OutfitViewer({
  ...
  // ── MODE IMAGE : rotation CSS sur l'image directement ──────────────────
  useEffect(() => {
    ...
    const animate = () => {
      angle += autoRotateSpeed * 0.4
      // On utilise une perspective + rotateY pour tourner l'image sur elle-même
      // quand l'image est "de dos" (cos < 0) on réduit l'opacité pour un effet naturel
      const cosA = Math.cos((angle * Math.PI) / 180)
      const scale = Math.abs(cosA)
      const opacity = 0.15 + 0.85 * Math.abs(cosA)

      if (imgRef.current) {
        imgRef.current.style.transform = `scaleX(${cosA})`
  ...
  // ── MODE MANNEQUIN 3D : Three.js ───────────────────────────────────────
  useEffect(() => {
    if (imageUrl) return // pas de Three.js si image
    ...
    const init = async () => {
      ...
      const H = height ?? container.clientHeight ?? 560
      canvas.width  = W
      canvas.height = H
      ...
      if (style === 'dress') {
        ...
        add(new THREE.SphereGeometry(0.11, 14,10), fab, 0.31,1.80,0)
      ...
      const pts = new THREE.Points(geo, new THREE.PointsMaterial({
        color: accentColor, size: 0.026, transparent: true, opacity: 0.6
      }))
      scene.add(pts)
  ...
  }, [style, fabricColor, accentColor, shoeColor, heelColor, imageUrl])

  // ── MODE IMAGE : div avec image + rotation CSS scaleX ──
  if (imageUrl) {
    return (
      <div ref={containerRef} className={`relative overflow-hidden ${className}`}>
        {/* Particules CSS dorées */}
